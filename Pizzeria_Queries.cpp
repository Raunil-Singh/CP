...
void solve(int tc_no) {
    ...
    vector<int> vr{all(v)}, vl{all(v)};
    for(int i = 0 ; i<n ; i++) {
        vr[i] += i;
        vl[i] -= i;
    }

    segment_tree segl(vl), segr(vr);
    ...
    while(q--) {
        ...
        if(i == 1) {
            int ind, x;
            cin >> ind >> x; ind--;
            segl.update(ind, x-ind);
            segr.update(ind, x+ind);
        } else {
            int ind; cin >> ind; ind--;
            cout << min(segl.query(0, ind) + ind, segr.query(ind, n-1) - ind) << "\n";
        }
    }
