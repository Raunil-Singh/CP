...
class lazy_segment_tree {
    ...
    int query_op(int a, int b) { return (a + b) % MOD; }
    int update_op(int a, int b) { return ((a % MOD) * (b % MOD)) % MOD; }
    ...
    void update(int l, int r, int v, int curr, int L, int R) {
        if(R < l || r < L) return;
        if(l <= L && R <= r) {
            seg[curr].update = update_op(seg[curr].update, v);
            seg[curr].query = update_op(seg[curr].query, v);
            return;
        }
        int mid = (L+R)/2;
