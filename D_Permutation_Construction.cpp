...
void solve(int tc_no) {
    ...
    ordered_set<int> elements(all(sum));
    ordered_set<int> perm;
    for(int i = 1 ; i<=n ; i++) perm.insert(i);

    vector<int> res(n);
    for(int i = n-1 ; i>=0 ; i--) {
        int lesser = elements.order_of_key(sum[i]);
        int tar = i - lesser;
        auto x = perm.find_by_order(tar);
        res[i] = *x;
        perm.erase(x);
        elements.erase(elements.find_by_order(lesser));
    }
