...
struct node{
    int pp, pn, np, nn;
    node() : pp(INT_MIN), pn(INT_MIN), np(INT_MIN), nn(INT_MIN) {}
    node(int x) : pp(x), pn(INT_MIN), np(INT_MIN), nn(-x) {}
    node(int a, int b, int c, int d) : pp(a), pn(b), np(c), nn(d) {}
};
...
class segment_tree {
    ...
    dt rule(const dt &a, const dt &b) {
        return {
            max(max(a.pp + b.np, a.pn + b.pp), max(a.pp, b.pp)),
            max(max(a.pp + b.nn, a.pn + b.pn), max(a.pn, b.pn)),
            max(max(a.np + b.np, a.nn + b.pp), max(a.np, b.np)),
            max(max(a.np + b.nn, a.nn + b.pn), max(a.nn, b.nn))
        };
    } // Rule to find the value of a node based on children

    dt init_create(int num) { return node(num); } // Function to decide the info stored on nodes
