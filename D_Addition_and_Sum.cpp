...
class lazy_segment_tree {
    ...
    int update_op(int a, int b, int l, int r, int L, int R) { return a + b * max((int)0, min(r, R) - max(l, L) + 1); }
    ...
    void update(int l, int r, int v, int curr, int L, int R) {
        if(R < l || r < L) return;
        if(l <= L && R <= r) {
            seg[curr].update = update_op(seg[curr].update, v, 1, 1, 1, 1); // update if needed
        ...
        seg[curr].query = update_op(query_op(seg[curr*2].query, 
                                                seg[curr*2 + 1].query),
    ...
    int query(int l, int r, int curr, int L, int R) {
        ...
        int MID = (L+R)/2;
        return update_op(query_op(query(l, r, curr*2, L, MID),
                                    query(l, r, curr*2+1, MID+1, R)),
                        seg[curr].update, l, r, L, R);
    }
