...
void recursion(int n, int k, int pos)
{
    if(k<0) return;
    if(pos < 0)
    {
    ...
    if(n&num)
    {
        count = nCr(pos, k);
        // cout << count << " " << ans << "->";
        ans = (ans + ((num-1)%MOD)*(count - nCr(pos-1, k) + MOD)) % MOD;
        int temp = n & (((1LL<<63)-1) - (2*num - 1));
        ans = (ans + (temp % MOD) * count) % MOD;
        // cout << ans << " " << pos << "\n";
    }
    recursion(n, k-((n&num)>0), pos-1);
}
