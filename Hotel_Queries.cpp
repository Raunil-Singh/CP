...
class segment_tree {
    ...
    int get_ans_(int tar, int curr, int l, int r) {
        if(l == r) return l;
        int mid = (l+r)/2;
        if(seg[curr*2] >= tar) return get_ans_(tar, curr*2, l, mid);
        else return get_ans_(tar, curr*2+1, mid+1, r);
    }
...
public:
    ...
    int get_ans(int tar) {
        int ans = -1;
        if(seg[1] >= tar) ans = get_ans_(tar, 1, 0, sze-1);
        return ans;
    }
...
void solve(int tc_no) {
    ...
    while(q--) {
        int x;
        cin >> x;
        int ans = seg.get_ans(x);
        if(ans != -1) seg.update(ans, seg.query(ans, ans) - x);
        cout << ans + 1 << " ";
    }
