...
void solve(int tc_no) {
    ...
    vector<vector<int>> queries(q); // duration, start, id
    vector<pair<int, int>> ranges(n); // duration, start

    vector<int> temp(MAX_LIM+10);
    segment_tree done(temp), yet(temp);
    ...
    for(auto &query : queries) {
        while(cr < n && ranges[cr].first <= query[0]) {
            int st = ranges[cr].second, en = ranges[cr].first + ranges[cr].second - 1;
            done.update(en, done.query(en, en) + 1);
            yet.update(st, yet.query(st, st) + 1);
            cr++;
        }
        int st = query[1], en = query[0] + query[1] - 1;
        ans[query[2]] = done.query(0, en) + yet.query(st, MAX_LIM) - cr;
        cout << query[2] << ": " << done.query(0, en) << " " << yet.query(st, MAX_LIM) << " " << cr << "\n";
    }
