...
class segment_tree {
    ...
    typedef struct { int sum, pref; } dt;
    ...
    dt init_create(int num) { return {num, max((int)0, num)}; }
...
public:
    segment_tree(vector<int> &v) : sze(v.size()), seg(4*v.size()), default_val{0, INT_MIN * (1LL<<20)} {
        build(v, 1, 0, sze-1);
    }
